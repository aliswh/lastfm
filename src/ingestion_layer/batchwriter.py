...
class BatchWriter(Writer):
    ...
    def write(self, dest_path, seed_user, users_limit, tracks_limit, debug = False):
        ...
        for user in users:
            if debug:
                print(f'User:{user}')
            self.dest.write(user, f"users/user_{user['id']}")
            recent_tracks = self.dest.get('recent_tracks',user['user'],limit=tracks_limit)
            self.dest.write(recent_tracks, f"recent_tracks/user_recent_tracks_{user['id']}")
            for track in recent_tracks[1:]:
                ...
                self.dest.write(track, f"tracks/track_{track['id']}")
                self.dest.write(artist, f"artists/artist_{artist['id']}")
                if debug:
                    print(f'Track:{track}')
